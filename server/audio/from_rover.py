...
def request_handler(request):
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS from_rover_table (encoded_audio text, time timestamp);''')
    c.execute('''CREATE TABLE IF NOT EXISTS is_new_table (new bool, time timestamp);''')
    
    now = datetime.datetime.now()
    if request["method"] == "GET":
        return c.execute('''SELECT * FROM from_rover_table''').fetchall()
        ...
        if "new" in request["args"]:
            ...
            if new:
                ...
                new_audio = c.execute('''SELECT * FROM from_rover_table ORDER BY time DESC''').fetchone()[0]
                conn.commit()
                conn.close()
                return new_audio
            else:
            ...
            c.execute('''INSERT into from_rover_table VALUES (?,?);''',(encoding, now))
