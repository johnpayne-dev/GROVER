...
const uint8_t PIN_2 = 39; //button to toggle listening mode
...
void loop() {
    ...
    int input_39 = digitalRead(BUTTON_39);
    switch(state_39) { //toggle listen
        case UP:
            if (! input_39) { //pressed
                state_39 = DOWN;
            }
    ...
    if (! listening) {
        ...
        if (value > threshold) {
            ...
            if (!conn) {
                ...
                client.print("POST /sandbox/sc/team24/GROVER/server/audio/from_controller.py"); client.print(" HTTP/1.1\r\n");
                client.print("Host: 608dev-2.net\r\n");
                client.print("Content-Type: application/json\r\n");
...
void record_audio() {
  ...
  while (millis() - loud_timer < 1000 && sample_num < NUM_SAMPLES) { //read in NUM_SAMPLES worth of audio data
    if (!digitalRead(BUTTON_39)) { //quits recording if listening mode is toggled mid audio
        quit = true;
        listening = false;
